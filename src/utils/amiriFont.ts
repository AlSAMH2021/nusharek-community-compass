 ...
 * 
 * IMPORTANT: We ONLY use reshaping to connect Arabic letters properly.
 * We do NOT reverse or reorder text because jsPDF handles RTL automatically
 * when using align: "right"
 */
export function processArabicText(text: string): string {
  if (!text) return text;
  
  try {
    // Only reshape Arabic text to handle proper character connections (ligatures)
    // Do NOT reverse or reorder - jsPDF handles RTL with align: "right"
 ...
 * Same as processArabicText - just reshape, no reordering
 */
